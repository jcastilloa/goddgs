...
| DuckDuckGo / `bing` | POST `https://html.duckduckgo.com/html/`; form en orden `q`, `b=""`, `l=region`, después `s=10+(page-2)*15` sólo si `page>1`, y `df=timelimit` sólo si es truthy. Ignora safesearch. Usa `HttpClient2`: TLS verificado (política/cifrados) se decide al construir el cliente; los siete SETTINGS H2 se deciden sólo al abrir conexión. | `//div[contains(@class,'body')]`; h2, href `./a`, body `./a`; elimina href que empieza `https://duckduckgo.com/y.js?`. Un 200 con `response.text == ""` devuelve `None`; DOM vacío o malformado no vacío devuelve `[]`. |
...
| Startpage / `google` | Primero separa región (`country-language`), después GET bootstrap `https://www.startpage.com/` para primer input `sc`, y POST `https://www.startpage.com/sp/search`; Referer explícito; campos fijos `cat`, `t`, `lui`, `language`, `abp`, `abd`, `abe`, `segment`; `qsr`, `qadf`, `sc`, página/time. Safesearch se mapea sólo después de bootstrap. | `//div[contains(@class,'result')][./a]`; h2, href directo, p. Bootstrap usa texto aunque status no sea 200; texto vacío lanza `ParserError`, HTML malformado puede dejar `sc=""` y continuar. El `_sc` se guarda pero no se reutiliza. |
...
| Yahoo / `bing` | GET URL nueva por búsqueda con `_ylt=token_urlsafe(18)` y `_ylu=token_urlsafe(35)`; `p`; página `b=(page-1)*7+1` sólo si >1; `btf` sólo truthy. Ignora región/safesearch. | `relsrch`; elimina Bing aclick; resultado ya normalizado con `/RU=` se corta en `/RK=` o `/RS=`, pasa `unquote_plus` y se normaliza otra vez; literal `+` e escapes inválidos son observables. |
| Yandex / `yandex` | GET `https://yandex.com/search/site/`; `text`, `web=1`, `searchid` aleatorio inclusivo 1000000..9999999; página `p=page-1` sólo si >1. Ignora región, safesearch y timelimit. | `serp-item`; h3, h3/a, div text. |
