...
| Q-47 | `HttpClient` delegates to randomized `primp` browser impersonation; `HttpClient2` randomizes TLS choices and monkey-patches HTTP/2 settings per request. | Base client chooses one full frozen browser/OS bundle from 23×5 outcomes at construction: TLS, headers and HTTP/2 remain coherent for its lifetime and cross direct/PEM/verify/CONNECT/SOCKS HTTPS target routes. DDG temporary-client fingerprint remains distinct; do not merge it into base behavior. |
...
| Q-41 | DuckDuckGo text enables HTTP/2, disables redirect follow, randomizes TLS/H2 settings, and globally monkey-patches `httpcore` H2 init around request. | `internal/transport.DuckDuckGoTextClient` proves request-local standard H2/no-redirect/header behavior and never recreates the global patch. Its randomized TLS/H2 fingerprint remains non-equivalent. |
