...
| Q-47 | `HttpClient` delegates to randomized `primp` browser impersonation; `HttpClient2` randomizes TLS choices and monkey-patches HTTP/2 settings per request. A controlled 2026-07-25 observation shows both Go clients negotiate HTTP/2 but have distinct JA3/JA4/Akamai hashes from their Python sources. | Do not claim transport or engine 1:1 parity from request fixtures/HTTP2 negotiation. Keep every affected engine incomplete until a reviewed request-local fingerprint transport has matching evidence. |
