...
| Q-47 | `HttpClient` delegates to randomized `primp` browser impersonation; `HttpClient2` shuffles SSL ciphers and samples one policy in its constructor, then temporarily patches HTTP/2 initialization around a request. The patched code runs only for a **new** `httpcore` H2 connection. | Base client chooses one full frozen browser/OS bundle from 23×5 outcomes at construction. DDG keeps its distinct client: enabled-verification TLS policy and legacy-cipher order are session/client lifetime (`verify=false` bypasses this draw); seven H2 values are connection lifetime, not request lifetime. Do not merge either client or create a Go-global patch. |
...
| Q-39c | Grokipedia uses `dict.get` only for absent `title`/`snippet`; explicit `null` then raises source `AttributeError`/`TypeError`. Its `slug` is an f-string, so JSON object input order and Python `str`/`repr` are URL-visible. Python `json.loads` accepts `NaN`/`Infinity` and exponent overflow as floats. | Preserve absent-versus-null/type errors and ordered Python-like slug rendering. Keep finite JSON numbers lexical until the adapter needs source float behavior; materialize only observed Python non-finite values. |
| Q-39d | Wikipedia derives language with exact two-part `region.lower().split("-")`; source `next(iter(query.pages.values()))` selects first JSON member insertion order, not lexicographic key. Its filter is the lowercase exact substring `may refer to:`. | Keep language/request state per call, retain ordered JSON objects for `pages`, preserve source `ValueError` and `StopIteration` cases, and do not case-fold the disambiguation filter. |
| Q-39e | Brave constructs cookies with a Python `dict`: country is lower-cased, a duplicate static/dynamic key is replaced in its original insertion position, and `safesearch` is added only when input differs exactly from `moderate`. | Preserve source dictionary replacement/order; do not emit duplicated cookie fields or normalize safesearch before this branch. |
| Q-39f | Google lower-cases only safesearch; it preserves country/language case while building `hl`, `lr`, `cr`, accepts page zero as `start=-10`, and normalizes then unwraps `/url?q=` before filtering title/non-HTTP results. | Do not ISO-normalize region, clamp page, or filter before source URL normalization/unwrapping. |
| Q-39g | Mojeek lower-cases region but accepts only exact lower-case safesearch `on`; it ignores timelimit and ignores page values `<=1`. | Keep source exact-string and page branches; do not share a generic safesearch/timelimit mapper. |
| Q-39h | Startpage splits region before its mandatory home-page bootstrap, but looks up safesearch only afterwards. Bootstrap consumes raw response text regardless of status: empty text raises `ParserError: Document is empty`, while malformed nonempty HTML can yield `sc=""` and continue to POST. | Preserve request/error ordering; do not pre-validate safesearch, require a 200 bootstrap, or turn absent `sc` into a default token. |
| Q-39i | Yahoo first normalizes the XPath href, filters Bing aclick, then strips `/RU=` delimiters, applies `unquote_plus`, and assigns the href again (normalizing twice). Literal `+`, `%2B`, `%ZZ`, and invalid UTF-8 therefore have source-visible outcomes. | Preserve exact post-processing order; do not substitute a generic URL unwrap or a single query-unescape pass. |
| Q-39j | Yandex ignores region, safesearch, and timelimit; it generates inclusive random `searchid` per search and omits `p` for page values `<=1`. | Keep random seam request-local and do not route generic option validation/mapping into Yandex. |
| Q-39k | Bing Images applies Python `int()` directly to engine-only `max_results`; it accepts Unicode decimal digits, `_`, and Python whitespace, clamps to 35, and raises `ValueError` for an empty/invalid value. Metadata `m=null` raises `AttributeError`; dimensions use Python whitespace after `×` replacement. | Preserve direct engine behavior behind ordered source parameters. Public max-results remains scheduler-only; do not clamp/coerce it into the engine path. |
| Q-39l | DuckDuckGo Images maps timelimit before VQD bootstrap but validates safesearch after it. Its six filter slots preserve empty holes, and `json_data.get("results", [])` exposes Python iterable/attribute failures for null/bool/nonempty object/string values. | Preserve request/error ordering, exact `f` slot layout, raw dynamic fields, and nil/empty/error distinctions; do not normalize malformed result roots. |
| Q-50 | `DDGS._get_engines` first selects keys, then creates/caches every selected engine class in source order before `_search_sync` submits workers. Cache entries persist between operations and each Python engine owns its own client; DuckDuckGo text additionally chooses a weighted fake-useragent value at class import. | Do not construct adapters in scheduler workers or share a generic transport across engine cache entries. Keep selection lazy between operations, construct selected adapters eagerly before fan-out, retain per-client cache synchronization, and use the reviewed full weighted DDG UA pool once per Go process. |
| Q-39m | Bing News splits region after lowercasing, maps only `d/w/m/y`, accepts nonpositive pages in `first=page*10+1`, parses absolute dates through the reference process local timezone before UTC, then matches localized relative days. | Preserve mapping/error order, local-time conversion, relative-date clock seam, and image prefix/truncation; do not clamp page or prevalidate ignored safesearch. |
| Q-39n | DuckDuckGo News fetches VQD before safesearch validation. It directly iterates `json_data.get("results", [])`: absent/empty dict/string yields `[]`; `null` and other shapes expose source `TypeError`/`AttributeError`. | Preserve bootstrap/error order and raw iterable behavior; do not coerce `results` to a list or silently skip dynamic items. |
| Q-39o | Yahoo News has one broad catch around its entire post-process loop. Its date clock is UTC with fixed 30-day months/365-day years; URL cleanup applies `/RU=`/`/RK=` and `unquote_plus` before BaseResult normalizes the URL again. | Keep the clock injected per adapter, preserve partial cleanup after the first malformed result, and do not turn source postprocess failures into returned errors. |
| Q-39p | DuckDuckGo Videos obtains VQD before safe-search lookup. Its `f` parameter always has four comma-separated slots, including empty slots; `json_data.get("results", [])` exposes Python iterable/attribute failures for null/nonempty object/string roots and null items. | Preserve VQD/error ordering, exact filter-slot layout, page stride 60, raw nested video values, and nil/empty/error distinctions. |

## Transport behavior

| ID | Frozen behavior | Port rule |
| --- | --- | --- |
| Q-40 | Default client uses `primp.Client` with random browser/OS impersonation. | Select browser variant and OS independently once per base client; preserve the resulting full bundle across direct and tunneled HTTPS. Plain HTTP must remain standard rather than receive browser headers alone. |
| Q-41 | DuckDuckGo text enables HTTP/2, disables redirect follow, randomizes TLS/H2 settings, and globally monkey-patches `httpcore` H2 init around request. | Preserve no-redirect/header/request behavior. Model source scope safely: select enabled-verification TLS policy once per isolated DDG client, choose H2 settings only while creating a connection, and never reproduce Python's package-global monkey patch. |
...
| Q-46 | Resolved `primp 1.3.1` renders `text_markdown`, `text_plain`, and `text_rich` through Rust `html2text 0.16.7` at width 100, using default, `TrivialDecorator`, and `RichDecorator` respectively. | Strict renderer parity is a known difference. The user approved the documented practical `html-to-markdown` renderer for these three formats; preserve source fetch/bytes/text/error behavior and never label the rendered strings `primp`-identical. |
