...
| Q-46 | Resolved `primp 1.3.1` renders `text_markdown`, `text_plain`, and `text_rich` through Rust `html2text 0.16.7` at width 100, using default, `TrivialDecorator`, and `RichDecorator` respectively. | Do not substitute a similarly named Go converter. It must match the frozen Markdown/plain/rich corpus exactly before extraction is implemented. |
